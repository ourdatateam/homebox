<template>
  <Dialog :dialog-id="DialogID.CameraCapture">
    <DialogScrollContent class="max-w-3xl" :class="{ 'h-screen max-w-full': isMobile }">
      <DialogHeader>
...
<script setup lang="ts">
  ...
  const { t } = useI18n();
  const { activeDialog, registerOpenDialogCallback, openDialog, closeDialog } = useDialog();
  ...
  const returnTo = ref<"create-item" | undefined>(undefined);
  ...
  onMounted(() => {
    const cleanup = registerOpenDialogCallback(DialogID.CameraCapture, params => {
  ...
  // When the active dialog flips away from CameraCapture (Esc, programmatic close,
  // or chained open of another DialogID), make sure the stream stops.
  watch(isOpen, open => {
    if (!open) {
      teardown();
    }
  ...
  function onDone() {
    ...
    const photos = [...captured.value];
    teardown();
    if (returnTo.value === "create-item") {
      // Re-open CreateItem WITHOUT closing first — openDialog swaps activeDialog atomically,
      // and CreateItem's registerOpenDialogCallback appends resumePhotos without resetting form fields.
      openDialog(DialogID.CreateItem, { params: { resumePhotos: photos } });
    } else {
      closeDialog(DialogID.CameraCapture, { photos });
    }
  }
  ...
  function onCancel() {
    teardown();
    if (returnTo.value === "create-item") {
      // Restore the CreateItem dialog so the user's in-progress form isn't visually lost.
